...
fn main() -> io::Result<()> {
    // // Uncomment to render the examples from the textbook Ray Tracing in One Weekend. Increase `samples_per_pixel` and `max_depth` for a higher quality image; reduce them for speed.
    ...
    // // Uncomment to render my custom examples.
    examples::other::cubes::render(10, 10)?;
    // examples::other::combo::render(10, 10, 800)?;

    // These are my versions of the scenes required for the 01Founders project.
    let max_depth = 50;
